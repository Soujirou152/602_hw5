...
public class CountFilesRec {
	...
	public static int countFiles(File dir) {
		...
		if (dir.isDirectory()) {
			...
			for (int i = 0; i < files.length; i++) // iterate over each file in
													// directory dir
			{
				if (files[i].isFile()) {
					count = count + 1;// add 1 to running count for ordinary
										// file,
				} else {
					count += countFiles(files[i]); // add 1+countFiles(sub) for
													// directory file
				}
			}
		}
